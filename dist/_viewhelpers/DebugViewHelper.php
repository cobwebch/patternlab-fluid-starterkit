...
class DebugViewHelper extends AbstractViewHelper
{
    ...
    public function initializeArguments()
    {
        ...
        $this->registerArgument('maxDepth', 'int',
            ...
            false, 8);
        $this->registerArgument('plainText', 'bool',
            'If TRUE, the dump is in plain text, if FALSE the debug output is in HTML format.', false, false);
        $this->registerArgument('ansiColors', 'bool',
            'If TRUE, ANSI color codes is added to the plaintext output, if FALSE (default) the plaintext debug output not colored.',
            false, false);
        $this->registerArgument('inline', 'bool',
            ...
            false, false);
        $this->registerArgument('blacklistedClassNames', 'array',
            'An array of class names (RegEx) to be filtered. Default is an array of some common class names.');
        $this->registerArgument('blacklistedPropertyNames', 'array',
    ...
    public static function renderStatic(
        ...
        return self::var_dump($renderChildrenClosure(), $arguments['title'], $arguments['maxDepth'],
            (bool)$arguments['plainText'], (bool)$arguments['ansiColors'], true,
            $arguments['blacklistedClassNames'], $arguments['blacklistedPropertyNames']);
    ...
    protected static function renderDump($value, $level, $plainText, $ansiColors)
    {
        ...
        if (is_string($value)) {
            ...
            if ($plainText) {
                $dump = self::ansiEscapeWrap(('"' . implode((PHP_EOL . str_repeat(self::PLAINTEXT_INDENT,
                                ($level + 1))), str_split($croppedValue, 76)) . '"'), '33',
                        $ansiColors) . ' (' . strlen($value) . ' chars)';
            } else {
                $lines = str_split($croppedValue, 76);
                $lines = array_map('htmlspecialchars', $lines);
                $dump = sprintf('\'<span class="extbase-debug-string">%s</span>\' (%s chars)',
                    implode('<br />' . str_repeat(self::HTML_INDENT, $level + 1), $lines), strlen($value));
            }
        } elseif (is_numeric($value)) {
            $dump = sprintf('%s (%s)', self::ansiEscapeWrap($value, '35', $ansiColors), gettype($value));
        } elseif (is_bool($value)) {
            $dump = $value ? self::ansiEscapeWrap('TRUE', '32', $ansiColors) : self::ansiEscapeWrap('FALSE', '32',
                $ansiColors);
    ...
    {
        $header = self::renderHeader($object, $level, $plainText, $ansiColors);
        if ($level < self::$maxDepth && $plainText) {
            $content = self::renderContent($object, $level, $plainText, $ansiColors);
        } else {
            $content = '';
        }
    ...
    protected static function renderHeader($object, $level, $plainText, $ansiColors)
    {
        ...
        if (!$object instanceof \Closure) {
            ...
            if ($plainText) {
                $dump .= ' ' . self::ansiEscapeWrap((($persistenceType ? $persistenceType . ' ' : '') . $domainObjectType),
                        '42;30', $ansiColors);
            } else {
        ...
        if ($object instanceof \Closure) {
            ...
            foreach ($reflectionFunction->getParameters() as $parameter) {
                ...
                if ($parameter->isDefaultValueAvailable()) {
                    $parameterDump .= ' = ';
                    if ($plainText) {
                        $parameterDump .= self::ansiEscapeWrap(var_export($parameter->getDefaultValue(), true), '33',
                            $ansiColors);
                    } else {
    ...
    protected static function renderCollection($collection, $level, $plainText, $ansiColors)
    {
        ...
        foreach ($collection as $key => $value) {
            $dump .= PHP_EOL . str_repeat(self::PLAINTEXT_INDENT,
                    $level) . ($plainText ? '' : '<span class="extbase-debug-property">') . self::ansiEscapeWrap($key,
